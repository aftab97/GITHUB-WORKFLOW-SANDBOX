  ...
  try {
    ...
    const messages = commits
      .map((commit) => {
        ...
        const match = commit.subject.match(
          /^Merge-pull-request-(\d+)-from-CG-GroupIT-(?:neo-bff|neo3-ui|notify)(?:-[^-]+)*-?(DC\d+-\d{2,4})(?:-.+)?$/,
        );
        if (match) {
          return {
            prNumber: match[1],
    ...
    for (const msg of messages) {
      try {
        ...
        changelogList.push(
          `- ${cleanTitle} ([Jira Ticket](https://e-3d-dc1.capgemini.com/jira/browse/${msg.ticket})) ([PR](https://github.com/CG-GroupIT/neo-monorepo/pull/${msg.prNumber}))`,
        );
      } catch (err) {
        console.warn(
